<template>
  <view class="container">
    <!-- 首屏骨架屏：仿提现记录单行版式（左侧标题条 + 两行说明条，右侧金额条），
         仅「请求中且列表为空」时出现，上拉翻页由已有列表继续展示（与教务课时流水列表同一口径） -->
    <tz-box padding="12px 12px" v-if="loading && !list.length">
      <view class="sk-cell" v-for="i in 4" :key="i">
        <view class="sk-cell-main">
          <u-skeleton
            :loading="true"
            :animate="true"
            :title="true"
    ...
    <tz-box padding="12px 12px" v-else-if="list.length > 0">
    ...
    <u-empty icon="/static/image/empty.png" width="70" height="70" v-else-if="!loading" text="暂无提现记录" />
...
export default {
  ...
  onLoad() {
    that = this;
    // 首屏反馈由列表骨架屏承担，不再叠加原生 loading 弹层
    this.getData();
  },
  onPullDownRefresh() {
    this.page = 1;
...
/* u-skeleton 外面套一层撑开剩余宽度：小程序端组件标签本身是块级宿主，直接作为 flex 项不会占满剩余空间 */
.sk-cell-main {
  flex: 1;
  min-width: 0;
}

/* 金额条：与真实金额徽标同档尺寸（14px 字号 + 2px 8px 内边距） */
.sk-amount {
  flex: none;
  width: 68px;
  height: 22px;
