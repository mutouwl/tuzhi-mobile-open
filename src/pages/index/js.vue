...
export default {
  data() {
    return {
      ...
      // 教务插件安装状态（付费插件）：未安装时模板不渲染课程套餐/老师/校区组件
      schoolInstalled: false,
    };
  },
  watch: {},
  methods: {
      ...
      that
        ...
        .then((res) => {
          ...
          if (res.code == 1) {
            ...
            // 教务组件安装状态：未安装时模板不渲染（数据由装修接口在插件已安装时下发）
            that.schoolInstalled = !!that.$tools.systemConfig("app_isinstall_school");
