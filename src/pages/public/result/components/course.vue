<template>
    <div class="dir-box" v-if="list.length > 0 || subscription">
        <div :class="['course-box']" v-for="(item, index) in list" :key="index"
            @click="jump(item.snapshoot.id, item.snapshoot.type, item.snapshoot.package_id)">
            <div class="cover">
            ...
            jump(id, type, packageId) {
                switch (type) {
                    ...
                    case "circle":
                        ...
                        break;
                    case "school_package":
                        //校务课程套餐：item_id 为规格 ID，跳转套餐详情需包 ID（未安装校务插件时不跳转）
                        if (packageId && this.$tools.systemConfig("app_isinstall_school")) {
                            this.$nav.to("/pages/app/school/package/detail/detail?id=" + packageId, "local", "redirectTo");
                        }
                        break;
