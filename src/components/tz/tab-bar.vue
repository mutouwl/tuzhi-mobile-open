...
<script>
	...
	export default {
		...
		methods: {
			...
			/**
			 * 归一化当前页路径：分享链接会带上 share/scene 等来源参数、首页路径会带空 id 参数，
			 * 它们影响页面归属判断但与导航项路径不一致，直接比对会导致底部导航不显示。
			 * 这里按参数逐个过滤后重建查询串——不能只做字符串删除：
			 * 去掉「?share=486」会把分隔符一起吃掉，留下「&id=」这种残缺串使后续匹配全部失效
			 */
			normalizePagePath(path) {
				...
				var basePath = path.substring(0, queryIndex);
				var kept = [];
				path.substring(queryIndex + 1).split('&').forEach(function (pair) {
					if (!pair) {
						return;
					}
					var key = pair.split('=')[0];
					var value = pair.substring(key.length + 1);
					// share：分享者 ID；scene：小程序码带来的分享参数
					if (key == 'share' && /^[0-9]*$/.test(value)) {
						return;
					}
					if (key == 'scene' && value.indexOf('share%') === 0) {
						return;
					}
					// 分类筛选、无值 id（首页默认页）不改变页面归属
					if (key == 'category_id' && /^[0-9]*$/.test(value)) {
						return;
					}
					if (key == 'id' && value === '') {
						return;
				...
				return kept.length ? basePath + '?' + kept.join('&') : basePath;
			},
			...
			setChecked(config) {
				...
				// 过滤来源参数后再与导航项路径比对
				var pagePath = this.normalizePagePath(path);
