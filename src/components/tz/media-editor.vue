<template>
	<view class="tz-media-editor">
		...
		<!-- show-add 时九宫格末尾追加添加格：即使一张都还没选也要把格子渲染出来 -->
		<tz-circle-media-grid class="editor-media-margin" v-if="gridItems.length || addVisible" :items="gridItems" editable :disabled="disabled" :show-add="addVisible" @preview-image="previewImage" @preview-video="previewVideo" @preview-audio="previewAudio" @remove="removeMediaItem" @add="emitAdd" />
...
<script>
/**
 * 媒体编辑器（图片/视频/音频/文件，发布动态/回复弹窗/表单页图片上传共用）
 ...
 * 上传入口两种放法，任选其一：
 *   1. 调用方自备按钮（发布页底部工具条），通过 ref 调用 chooseImage/chooseVideo/chooseAudio/chooseFile
 *   2. 传 show-add：九宫格末尾追加一个「＋」格，与已选媒体同行排、满三列换行，点击抛 add 事件由调用方选媒体
 *      （图片达到 max-images 上限后自动隐藏；上传中随 disabled 置灰且不可点）
...
export default {
	...
	props: {
		...
		// 在九宫格末尾追加添加格（点击抛 add 事件）：表单页只有图片上传时用它替代页面自备的添加按钮
	...
	computed: {
		...
		// 添加格实际显隐：传满 max-images 后隐藏；上传中的图片计入 pendingImages，
		// 与 chooseImage 的剩余名额同一口径，避免并发多选时添加格先消失/后消失不一致
		addVisible() {
			if (!this.showAdd) return false;
			const imageLimit = Number(this.maxImages);
			return !(imageLimit > 0 && this.media.images.length + this.pendingImages >= imageLimit);
		},
	},
	watch: {
	...
	methods: {
		...
		// 转发九宫格添加格的点击（微信小程序不在模板表达式里直接 $emit，跨组件转发用显式方法）
		emitAdd() {
			this.$emit('add');
		},
		...
		uploadFile(filePath, fileType, extra) {
			...
			uploadTask.then(res => {
				...
				uni.hideLoading();
				if (this.uploadingCount > 0) this.uploadingCount--;
				// 上传失败：释放待处理名额；失败原因 uploadImage 兜底已统一提示一次，此处不再重复 toast
