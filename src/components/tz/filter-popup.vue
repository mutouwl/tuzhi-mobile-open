<template>
  <view>
    <!-- 筛选弹窗（排序 + 时间）：圈子首页/圈子搜索页与教务流水/上课记录页共用。
         排序选项由 sortOptions 配置，传空数组隐藏排序组（仅时间筛选场景） -->
    <u-popup :show="visible" mode="bottom" :round="16" closeable :safeAreaInsetBottom="false" @close="onClose">
 ...
 * sortOptions 传空数组时隐藏排序组，用于仅时间筛选的页面（教务流水/上课记录）。
 */
...
export default {
  ...
  props: {
    ...
    // 弹窗标题（动态筛选/搜索筛选/流水筛选等）
    title: {
      type: String,
      default: '筛选',
    },
    // 当前生效排序（取值须在 sortOptions 内，默认 new）
    sort: {
      type: String,
      default: 'new',
    },
    // 排序选项（name/value），空数组隐藏排序组
    sortOptions: {
      type: Array,
...
.sort-popup {
  ...
  min-height: 300px;
  padding: 16px 14px;
  /* 底部安全区由弹层自身承担（模板已关 u-popup 的 safeAreaInsetBottom，此前由组件补一份全量 34px）；
     微信端按教务/全局底栏口径减半，H5 与改动前等量 */
  padding-bottom: calc(16px + constant(safe-area-inset-bottom));
  padding-bottom: calc(16px + env(safe-area-inset-bottom));
  /*  #ifdef  MP-WEIXIN  */
  padding-bottom: calc(16px + constant(safe-area-inset-bottom) / 2);
