<template>
	...
	<view class="post-media-grid" :class="{ 'is-editable': editable, 'is-content-indented': contentIndent && !editable }">
		...
		<!-- 添加格（调用方开启后排在媒体格末尾）：与媒体格同一 flex 流，右侧接着排、满三列自动换行；点击只抛 add，选什么媒体由调用方决定 -->
		<view v-if="showAdd" class="post-media-add" :class="{ 'is-disabled': disabled }" @tap.stop="onAddClick"><text class="post-media-add-icon">＋</text></view>
	</view>
</template>
 ...
 *        showAdd（末尾追加一个「＋」添加格，与媒体格同宽同流，默认 false）
 ...
 *        add（点击添加格，仅 emit 事件，选择什么媒体由调用方决定）
 */
export default {
	...
	props: {
		...
		// 末尾追加添加格（媒体编辑器用）：与媒体格排在同一 flex 流里
		showAdd: { type: Boolean, default: false },
	},
	emits: ['preview-image', 'preview-video', 'preview-audio', 'remove', 'add'],
	data() {
		return {
	...
	methods: {
		...
		// 点击添加格：禁用态（上传中）不响应，其余一律交给调用方（本组件不知道要选哪类媒体）
		onAddClick() {
...
/* 添加格：与媒体格同宽（三列公式）排在同一 flex 流里，接着最后一格往右排、满三列自动换行。
   box-sizing 必须 border-box：虚线边框不计入宽度，否则三格 + 两个间距会超出容器宽度提前换行 */
.post-media-add { width: calc((100% - 16px) / 3); height: 0; padding-bottom: calc((100% - 16px) / 3); border: 1px dashed #c9cdd4; border-radius: 8px; background: #f2f3f5; box-sizing: border-box; position: relative; }
/* height:0 + padding-bottom 撑起正方形，加号用绝对定位居中 */
.post-media-add-icon { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 24px; color: #86909c; }
/* 上传中禁用态：与圈子发布页工具按钮一致置灰 */
.post-media-add.is-disabled { opacity: 0.4; }
