<template>
    <div :class="[
        ...
        <img :src="currentCover" :alt="name" @error="imgError">
            <div class="type">
                <span>{{ $t(type) }}</span>
            </div>
        </div>
...
// #ifdef MP-TOUTIAO
    ...
    props: {
        ...
        //课程套餐 ID（school_package 类型的 item_id 为规格 ID，跳转详情需包 ID）
        packageId: {
            type: [String, Number],
            default: ''
        },
    ...
    methods: {
        imgError() {
            if (this.currentCover !== this.defaultCover) {
        ...
        courseHandle() {
            ...
            switch (this.type) {
                ...
                case 'circle':
                    ...
                    break;
                case 'school_package':
                    //校务课程套餐：跳转套餐详情（未安装校务插件时不跳转）
                    if (this.packageId && this.$tools.systemConfig('app_isinstall_school')) {
                        this.$nav.to('/pages/app/school/package/detail/detail?id=' + this.packageId, 'local')
                    }
                    break;
    ...
    watch: {
        cover: {
            immediate: true,
            handler(val) {
                this.currentCover = val || this.defaultCover;
            }
        }
    },
    mounted() {
