<template>
  <view :id="attrs.id" :class="'_'+name+' '+attrs.class" :style="attrs.style">
    <block v-for="(n, i) in childs" v-bind:key="i">
      ...
      <!-- #ifdef H5 -->
      <video-play v-else-if="n.name=='video'" :domId="n.attrs.id" :source="n.src[ctrl[i]||0]" />
      <!-- #endif -->
      <!-- #ifdef MP -->
      ...
      <rich-text class="rich-text" v-else-if="handler.use(n)" :id="n.attrs.id" :style="n.f" :nodes="[n]" />
      <!-- #endif -->
      <!-- #ifndef H5 || MP-WEIXIN || MP-QQ || APP-PLUS || MP-360 -->
      <rich-text class="rich-text" v-else-if="!n.c" :id="n.attrs.id" :style="n.f+';display:inline'" :preview="false" :nodes="[n]" />
      <!-- #endif -->
      <!-- 继续递归 -->
