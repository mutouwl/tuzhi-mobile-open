...
// 说明：custom-style 中 zIndex 固定为 999。无 z-index 时页面内 position:fixed 且 DOM 靠后的元素（如发布页底部操作栏）
// 会盖在加载层之上，导致加载蒙层没有覆盖整页；加载层应始终位于页面内容之上（低于 u-popup 的 10075）。
// 注意：该对象内的注释会被编译进小程序 WXML 属性值导致编译报错，注释只能写在 template 之外。
</script>
