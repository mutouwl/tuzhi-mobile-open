...
export default {
	...
	// 手机号 3 4 4 格式展示（座机等非 11 位号码原样返回）
	formatPhone(phone = '') {
		let p = phone === null || phone === undefined ? '' : phone.toString()
	...
	callPhone(phoneNumber = '') {
		...
		uni.makePhoneCall({
			phoneNumber: num,
			fail(err) {
				console.log('makePhoneCall出错 - tools.js:112', err)
			},
		});
	},
	// 图片处理-选择图片
	...
	uploadImage(api, url, opts = {}) {
		...
		if (opts.directUpload === false) {
			return this.uploadServer(api, url, config_url, token).catch(e => {
				uni.hideLoading();
				uni.$u.toast((e && (e.msg || e.message)) || '上传失败');
				throw e;
			});
		}
		...
		return this.getUploadConfig(true).then(cfg => {
			...
			if (strategy === 'oss') {
				// 直传失败自动回退服务器中转：云存储开关在服务器缓存里未及时刷新、AccessKey 被禁用、
				// 跨域被拦等场景都不该让用户提交不了表单；回退仍失败时由外层 catch 提示一次真实原因
				return this.uploadOss(url, opts).catch(() => this.uploadServer(api, url, config_url, token));
			}
			...
			uni.uploadFile({
				...
				success: res => {
					...
					if (data && data.code === 1) {
						...
						// 失败原因放 Error 里由 uploadImage 兜底 catch 统一提示一次，此处不重复 toast
						uni.hideLoading();
	...
	uploadOss(url, opts) {
		...
		return api('upload.ossParams', { name, md5: '', group: 'circle' }).then(res => {
			if (!res || res.code !== 1 || !res.data) {
				// 后台未开启直传（含配置刚改、服务器缓存未刷）会在这里返回原因；
				// 该错误会被 uploadImage 的兜底 catch 接住并回退服务器中转，不会弹给用户
				throw new Error((res && res.msg) || '获取OSS签名失败');
			}
			const p = res.data;
			return new Promise((resolve, reject) => {
				uni.uploadFile({
					...
					success: (uploadRes) => {
						// OSS 直传失败（签名/策略/凭证失效等）同样会走 success 回调，必须按状态码区分：
						// 否则文件未落桶也提示成功，页面拿到的是访问 404 的无效地址
						if (!uploadRes || Number(uploadRes.statusCode) < 200 || Number(uploadRes.statusCode) >= 300) {
							const body = String((uploadRes && uploadRes.data) || '');
							const ossCode = (body.match(/<Code>([^<]+)<\/Code>/i) || [])[1];
							reject(new Error(ossCode ? 'OSS直传失败：' + ossCode : 'OSS直传失败，HTTP ' + ((uploadRes && uploadRes.statusCode) || 0)));
							return;
						}
						// 附件补记（服务端幂等）：OSS 回调不可达的部署由客户端补记，同 VOD 的 vodNotify 兜底；
						// 补记失败不阻塞上传结果（文件已落桶，仅影响附件管理记录）
						api('upload.ossNotify', { url: p.key, name: name }).catch(() => {});
						uni.hideLoading();
				...
				uni.uploadFile({
					...
					success: (uploadRes) => {
						...
						api('upload.vodNotify', {
							...
							.catch((err) => {
								uni.hideLoading();
								// 失败原因由 uploadImage 兜底 catch 统一提示一次，此处不重复 toast
