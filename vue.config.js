...
class FixTdesignMpPlugin {
  ...
  injectStyleIsolation(tdRoot) {
    ...
    // 递归遍历：除顶层组件外，嵌套子组件（如 calendar/template、calendar/calendar-header）
    // 也要注入 styleIsolation: shared —— uni-app 编译不会把组件 options 里的 styleIsolation
    // 写进产物 .json，缺了它微信默认 isolated，页面样式（如日历弹窗标题靠左、主题 CSS 变量）
    // 无法作用到子组件内部。TDesign 源码本就为所有组件声明了 styleIsolation: shared，此处只是补齐编译丢失的声明。
    for (const entry of fs.readdirSync(tdRoot, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue
      const dirPath = path.join(tdRoot, entry.name)
      this.injectStyleIsolationDir(dirPath)
    }
  }

  injectStyleIsolationDir(dirPath) {
    for (const entry of fs.readdirSync(dirPath, { withFileTypes: true })) {
      if (entry.isDirectory()) {
        this.injectStyleIsolationDir(path.join(dirPath, entry.name))
        continue
      }
      ...
      // 仅处理组件 json（非组件的 json 如 package.json 不动）
  ...
  walkAndFix(dir) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        // /../../../../Users/<you>/node-modules/@tdesign/uniapp/...。
...
module.exports = {
  ...
  chainWebpack: config => {
    // 让 webpack 解析时可找到 .ts 文件（@tdesign/uniapp 的 dist 内以 TS 源码发布）
    config.resolve.extensions.add('.ts').add('.mjs')

    // 用 babel 编译 @tdesign/uniapp 包内被引用的 .ts 文件（如 props.ts、common/validator.ts、locale/*.ts），
    // 仅编译 tdesign-uniapp 包，避免影响 src 下其它 TS 文件的既有处理方式。
    config.module
      .rule('ts')
      .test(/\.ts$/)
      .include
        .add(path.resolve(__dirname, 'node_modules', '@tdesign', 'uniapp'))
        .end()
      .use('babel-loader')
      .loader('babel-loader')
      .options({
