...
| 项目 | 内容 |
| --- | --- |
| API Key | `user.withdraw.saveAccount` |
| 接口地址 | `POST /api/user/withdraw/saveAccount` |
| 请求方式 | POST |
| 是否需登录 | 是 |
| 所属模块 | 用户（user） |
...
```bash
curl -X POST '{apiUrl}user/withdraw/saveAccount' \
  -H 'Content-Type: application/json' \
  -d '{"id":0,"type":"xxx","name":"xxx","account":"xxx","ext":""}'
```
