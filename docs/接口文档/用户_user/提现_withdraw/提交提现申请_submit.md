...
| 项目 | 内容 |
| --- | --- |
| API Key | `user.withdraw.submit` |
| 接口地址 | `POST /api/user/withdraw/submit` |
| 请求方式 | POST |
| 是否需登录 | 是 |
| 所属模块 | 用户（user） |
...
```bash
curl -X POST '{apiUrl}user/withdraw/submit' \
  -H 'Content-Type: application/json' \
  -d '{"money":"xxx","type":"bank","user_name":""}'
```
