...
> 获取练习分组树（4级，仅启用） 结构：[{id,name,status,sort,parent_id,level,children:[...]}]

## 基本信息

| 项目 | 内容 |
| --- | --- |
| API Key | `app.exam.exercises.groupTree` |
| 接口地址 | `POST /api/app/exam/exercises/groupTree` |
| 请求方式 | POST |
| 是否需登录 | 否 |
| 所属模块 | 拓展应用（app） |
| 所属控制器 | Exercises（练习） |
...
```bash
curl -X POST '{apiUrl}app/exam/exercises/groupTree' \
